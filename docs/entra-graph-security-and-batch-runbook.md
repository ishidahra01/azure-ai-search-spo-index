...
2. **Tenant A でアプリ設定**
   ...
   - `Sites.Selected` を追加
   - 「管理者の同意を与えます」は**ここでは実行しない**（権限は後述の手順3で Tenant B 側の管理者同意により付与されるため、Tenant A 側での同意は不要）
...
| **マルチテナント設定** | 必須<br/>（クロステナント認証のため） | 必須<br/>（クロステナント認証のため） |
...
> 注記: どちらの方式でも、Tenant A と Tenant B が異なる場合は、クロステナント認証を実現するためにマルチテナント設定が必須です。
